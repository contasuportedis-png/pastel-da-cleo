...
function finishOrder(){
  ...
  DB.orders.push({id:'PED-'+Date.now().toString().slice(-6),itens:cartDetailed().map(i=>({pid:String(i.p.id||'').slice(0,80),nome:String(i.p.nome).slice(0,80),qty:Math.max(1,Math.min(99,i.qty|0)),sub:i.sub})),total:cartSubtotal()+(checkout.tipo==='delivery'?(deliveryFee()||0):0),fee:checkout.tipo==='delivery'?(deliveryFee()||0):0,tipo:checkout.tipo,nome:checkout.nome,pag:checkout.pag,status:'Novo',data:new Date().toISOString()});
